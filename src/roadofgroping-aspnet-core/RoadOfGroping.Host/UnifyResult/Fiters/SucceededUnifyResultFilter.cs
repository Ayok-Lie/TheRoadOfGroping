...
namespace RoadOfGroping.Host.UnifyResult.Fiters
{
    ...
    public class SucceededUnifyResultFilter : IAsyncActionFilter, IOrderedFilter
    {
        ...
        internal static bool CheckStatusCodeNonUnify(HttpContext context, out IRESTfulResultProvider unifyResult)
        {
            ...
            // 判断是否跳过规范化处理
            var isSkip = context.GetEndpoint()?.Metadata?.GetMetadata<SkipActionFilterAttribute>() != null
                         || endpointFeature?.Endpoint?.Metadata?.GetMetadata<SkipActionFilterAttribute>() != null
